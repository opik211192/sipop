...
<body>
    <div class="container-fluid">
        ...
        <div id="uploadSection">
            <?php
            $tahapanBlanko3c = $jaringan->tahapans->where('nama_tahapan', 'Evaluasi Awal Kesiapan')->first();
            $dokumenBlanko3c = $tahapanBlanko3c ? $tahapanBlanko3c->dokumens->where('nama_dokumen', 'Blanko 3C')->first() : null;
            ?>
            @if ($dokumenBlanko3c)
            <!-- Tombol Show Dokumen -->
            <button class="btn btn-primary" data-toggle="modal" data-target="#showDokumenModal">
                <span class="fas fa-eye" title="Lihat Blanko 3C"></span> Lihat Dokumen
            </button>
            <!-- Tombol Delete Dokumen -->
            @can('manage blanko')
            <button class="btn btn-danger" id="deleteDokumenBtn">
                <span class="fas fa-trash" title="Hapus Blanko 3C"></span> Hapus Dokumen
            </button>
            @endcan
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="showDokumenLabel">Lihat Dokumen Blanko 3C</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <iframe src="{{ asset('storage/blanko3c/' . basename($dokumenBlanko3c->path_dokumen)) }}"
            ...
            <script>
                ...
                $('#deleteDokumenBtn').on('click', function () {
                    if (confirm('Apakah Anda yakin ingin menghapus dokumen ini?')) {
                        $.ajax({
                            url: '{{ route("delete-blanko3c", $jaringan->id) }}',
                            type: 'DELETE',
                            data: {
                                _token: '{{ csrf_token() }}'
                            },
            ...
            @can('manage blanko')                
            <div class="col-md-6">
                <form id="upload-blanko3c-form" action="{{ route('upload-blanko3c', $jaringan->id) }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label for="blanko3c" class="form-label">Unggah Blanko 3C</label>
                        <div class="input-group">
                            <input type="file" class="form-control" id="blanko3c" name="blanko3c">
        ...
        <form id="blanko3c-form"
            action="{{ route('inventarisasi-awal-kesiapan-manajemen-proses', ['jaringan' => $jaringan->id]) }}"
            method="POST">
            @csrf
            @method('PUT')
            <div class="card">
                <div class="card-body bg-light">
                    <table class="table table-bordered table-sm">
                        <thead>
                            <tr>
                                <th>No.</th>
                                <th>Jenis Kegiatan Manajemen</th>
                                <th>Rincian</th>
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="items[{{ $item->id }}][rincian][{{ $rincian->id }}][ada_tidak_ada]"
                                        class="form-control" style="width: 120px" onchange="calculateWeights()" @cannot('manage blanko') disabled  @endcannot>
                                ...
                                <td class="d-none">
                                    <input type="text"
                                        ...
                                        oninput="validateAndConvert(this)" @cannot('manage blanko')
                                            readonly
                                        @endcannot>
                                </td>
                                <td class="d-none">
    ...
    <script>
        ...
        $('#upload-blanko3c-form').on('submit', function(e) {
            e.preventDefault();
                            ...
                            <div class="modal fade" id="showDokumenModal" tabindex="-1" role="dialog" aria-labelledby="showDokumenLabel"
                                ...
                                <div class="modal-dialog modal-lg" role="document">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <iframe src="/storage/blanko3c/${response.fileName}" width="100%" height="400px"></iframe>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        $(document).ready(function() {
            ...
            $('#blanko3c-form').on('submit', function(e) {
