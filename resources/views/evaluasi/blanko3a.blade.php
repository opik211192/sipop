...
<body>
    <div class="container-fluid">
        ...
        <div id="uploadSection">
            <?php
            $tahapanBlanko3a = $jaringan->tahapans->where('nama_tahapan', 'Evaluasi Awal Kesiapan')->first();
            $dokumenBlanko3a = $tahapanBlanko3a ? $tahapanBlanko3a->dokumens->where('nama_dokumen', 'Blanko 3A')->first() : null;
            ?>
            @if ($dokumenBlanko3a)
            <!-- Tombol Show Dokumen -->
            <button class="btn btn-primary" data-toggle="modal" data-target="#showDokumenModal">
                <span class="fas fa-eye" title="Lihat Blanko 3A"></span> Lihat Dokumen
            </button>
            <!-- Tombol Delete Dokumen -->
            @can('manage blanko')
            <button class="btn btn-danger" id="deleteDokumenBtn">
                <span class="fas fa-trash" title="Hapus Blanko 3A"></span> Hapus Dokumen
            </button>
            @endcan
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="showDokumenLabel">Lihat Dokumen Blanko 3A</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <iframe src="{{ asset('storage/blanko3a/' . basename($dokumenBlanko3a->path_dokumen)) }}"
            ...
            <script>
                ...
                $('#deleteDokumenBtn').on('click', function () {
                    if (confirm('Apakah Anda yakin ingin menghapus dokumen ini?')) {
                        $.ajax({
                            url: '{{ route("delete-blanko3a", $jaringan->id) }}',
                            type: 'DELETE',
                            data: {
                                _token: '{{ csrf_token() }}'
                            },
            ...
            @can('manage blanko')                
            <div class="col-md-6">
                <form id="upload-blanko3a-form" action="{{ route('upload-blanko3a', $jaringan->id) }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label for="blanko3a" class="form-label">Unggah Blanko 3A</label>
                        <div class="input-group">
                            <input type="file" class="form-control" id="blanko3a" name="blanko3a">
        ...
        <form id="blanko3a-form"
            action="{{ route('inventarisasi-awal-kesiapan-sarana-penunjang-op-proses', ['jaringan' => $jaringan->id]) }}"
            method="POST">
            @csrf
            @method('PUT')
            <div class="card">
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="items[{{ $item->id }}][rincian][{{ $rincian->id }}][ada_tidak_ada]"
                                        class="form-control" style="width: 120px" onchange="calculateWeights()" @cannot('manage blanko')
                                        disabled
                                        @endcannot>
                                ...
                                <td class="d-none">
                                    <input type="text"
                                        ...
                                        oninput="validateAndConvert(this)" onchange="calculateWeights()" @cannot('manage blanko')
                                        disabled
                                        @endcannot>
                                </td>
                                <td class="d-none">
                                ...
                                <td>
                                    <input type="text"
                                        ...
                                        class="form-control" value="{{ $rincian->keterangan }}" @cannot('manage blanko')
                                            disabled
                                        @endcannot>
                                </td>
                            </tr>
    ...
    <script>
        ...
        $('#upload-blanko3a-form').on('submit', function(e) {
            e.preventDefault();
                            ...
                            <div class="modal fade" id="showDokumenModal" tabindex="-1" role="dialog" aria-labelledby="showDokumenLabel"
                                ...
                                <div class="modal-dialog modal-lg" role="document">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <iframe src="/storage/blanko3a/${response.fileName}" width="100%" height="400px"></iframe>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        $(document).ready(function() {
            ...
            $('#blanko3a-form').on('submit', function(e) {
