...
<body>
    <div class="container-fluid">
        ...
        <div id="uploadSection" class="mb-3">
            <?php
            $tahapanBlanko3d = $jaringan->tahapans->where('nama_tahapan', 'Evaluasi Awal Kesiapan')->first();
            $dokumenBlanko3d = $tahapanBlanko3d ? $tahapanBlanko3d->dokumens->where('nama_dokumen', 'Blanko 3D')->first() : null;
            ?>
            @if ($dokumenBlanko3d)
            <!-- Tombol Show Dokumen -->
            <button class="btn btn-primary" data-toggle="modal" data-target="#showDokumenModal">
                <span class="fas fa-eye" title="Lihat Blanko 3D"></span> Lihat Dokumen
            </button>
            <!-- Tombol Delete Dokumen -->
            @can('manage blanko')
            <button class="btn btn-danger" id="deleteDokumenBtn">
                <span class="fas fa-trash" title="Hapus Blanko 3D"></span> Hapus Dokumen
            </button>
            @endcan
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="showDokumenLabel">Lihat Dokumen Blanko 3D</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <iframe src="{{ asset('storage/blanko3d/' . basename($dokumenBlanko3d->path_dokumen)) }}"
            ...
            <script>
                ...
                $('#deleteDokumenBtn').on('click', function () {
                    if (confirm('Apakah Anda yakin ingin menghapus dokumen ini?')) {
                        $.ajax({
                            url: '{{ route("delete-blanko3d", $jaringan->id) }}',
                            type: 'DELETE',
                            data: {
                                _token: '{{ csrf_token() }}'
                            },
            ...
            @can('manage blanko')                
            <div class="col-md-6">
                <form id="upload-blanko3d-form" action="{{ route('upload-blanko3d', $jaringan->id) }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label for="blanko3d" class="form-label">Upload Blanko 3D</label>
                        <div class="input-group">
                            <input type="file" class="form-control" id="blanko3d" name="blanko3d">
        ...
        <form id="blanko3d-form"
            ...
            <div class="form-group">
                ...
                <button type="button" class="btn btn-secondary mt-3 mb-2" onclick="window.close()"><span
                        class="fa fa-times"></span> Kembali</button>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        $('#upload-blanko3d-form').on('submit', function(e) {
            e.preventDefault();
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        $('#uploadSection').html(`
                            ...
                            <div class="modal fade" id="showDokumenModal" tabindex="-1" role="dialog" aria-labelledby="showDokumenLabel" aria-hidden="true">
                                <div class="modal-dialog modal-lg" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="showDokumenLabel">Lihat Dokumen Blanko 3D</h5>
                                        ...
                                        <div class="modal-body">
                                            <iframe src="/storage/blanko3d/${response.fileName}" width="100%" height="400px"></iframe>
                                        </div>
                                    </div>
                                </div>
                            </div>
